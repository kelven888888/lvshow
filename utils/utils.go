...
// start: 保留前面的字符数
// end: 保留后面的字符数
func MaskString(src string, start, end int) string {
	if src == "" {
		return ""
	}

	runeSrc := []rune(src) // 处理中文等多字节字符
	length := len(runeSrc)

	// 如果字符串长度小于需要保留的总长度，直接返回原串或全部掩码
	if length <= start+end {
	...
	// 构建脱敏字符串
	// 前部分 + 星号 + 后部分
	masked := string(runeSrc[:start]) + strings.Repeat("*", length-start-end) + string(runeSrc[length-end:])
	return masked
}
...
func GenerateOrderID() string {
	...
	return fmt.Sprintf("SG%s%d", nano[3:], randNum)
}
...
func Get_funtype_Key(types int) string {
	switch types {
	case Rechargetype:
		return "充值"
	case Adminlogtype:
		return "平台操作"
	case Withdrayapplytype:
		return "提现"
	case Withdrayapplyrefuetype:
		return "提现拒绝"
	case Lottery:
		return "抽奖"
	case BUYGOOD:
...
func GenerateRandomString(length int) string {
	...
	for i := range result {
		result[i] = chars[rand.Intn(len(chars))] // 生成一个随机索引并获取字符
	}
...
func ExtractDecimalPart(num float64) float64 {
	str := fmt.Sprintf("%f", num)
	parts := strings.Split(str, ".")
	if len(parts) > 1 {
		decimalPart, _ := strconv.ParseFloat(parts[1], 64)
		return decimalPart / math.Pow(10, float64(len(parts[1]))) // 根据小数点后的位数调整小数部分的值以匹配原始数字的精度
	}
	return 0 // 如果没有小数部分，返回0或者根据需要返回其他值
}
func GetDecimalPlaces(f float64) int {
	// 将浮点数转换为字符串，'-1' 表示使用最少的小数位来精确表示数字
	// 'f' 表示用浮点数格式
	...
	// 小数点后的位数 = 字符串总长度 - 小数点位置 - 1 (减去小数点本身)
