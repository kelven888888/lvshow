...
func (this *IndexController) Console(ctx *gin.Context) {
	if ctx.Request.Method == "GET" {
		...
		var orderdeal model.OrderDeal
		global.SHOP_DB.Where("quan_account_id>0").Order("id desc ").Limit(1).First(&orderdeal)
		...
		global.SHOP_DB.Model(model.MTradeAccountApply{}).Where("status='0'").Count(&undotradeapplyaccount)
		var unrechargecount int64
		global.SHOP_DB.Model(model.FundRecharge{}).Where("status='0'").Count(&unrechargecount)

		totalnum := undoWithdrawcount + undotradeapplyaccount + unrechargecount
		...
		ctx.HTML(http.StatusOK, "console.html", gin.H{
			...
			"quanordertime":         orderdeal.CreateTime.Format(time.DateTime),
			"marketopen":            marketopen,
			"undoWithdrawcount":     undoWithdrawcount,
			"undotradeapplyaccount": undotradeapplyaccount,
			"unrechargecount":       unrechargecount,
			"dates":                 dates,
