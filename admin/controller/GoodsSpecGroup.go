...
type CGoodsSpecGroup struct {
	Services service.SGoodsSpecGroup
	BaseController
}

func (this *CGoodsSpecGroup) Index(ctx *gin.Context) {
	...
	ctx.HTML(http.StatusOK, "goodsspecgroup_index.html", gin.H{
		"status": "200",
		"List":   result,
		"Count":  count,
		"Search": Search,
...
func (this *CGoodsSpecGroup) Edit(ctx *gin.Context) {
	...
	if ctx.Request.Method == "GET" {
		...
		restotal := this.getSelectTree(category, result.Cid, 0)
		global.SHOP_DB.Model(model.Language{}).Where("status=1").Find(&language)
		// 查询权限列表
		ctx.HTML(http.StatusOK, "goodsspecgroup_form.html", gin.H{
			"status":   "200",
			"result":   result,
			"language": language,
			"IsUpdate": true,
		...
		var models model.GoodsSpecGroup
...
func (this *CGoodsSpecGroup) Delete(ctx *gin.Context) {
...
func (this *CGoodsSpecGroup) Add(ctx *gin.Context) {
	if ctx.Request.Method == "GET" {
...
func (this *CGoodsSpecGroup) Deletebatch(ctx *gin.Context) {
...
// 格式化为select表单树
func (this *CGoodsSpecGroup) getSelectTree(categorys []*model.Category, pid int, level int) string {
	html := ""
