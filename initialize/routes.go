...
import (
	...
	api_v1 "ginshop.com/api/v1/route"
	"ginshop.com/global"
	"ginshop.com/middleware"
	statics "ginshop.com/public"
	"ginshop.com/router"
	"ginshop.com/utils"
	"ginshop.com/utils/gconv"
...
func IninRoute() *gin.Engine {
	...
	Router.Any("/cdn-cgi/rum", func(context *gin.Context) {
	...
	PrivateGroup.Use(middleware.AdminMiddleware()).Use(middleware.OperationRecord()).Use(middleware.ChecmAdmin())
	//PrivateGroup.Use(middleware.OperationRecord())
	{
		...
		router.InitBannerRoute(PrivateGroup)   // 注册功能api路由
		router.InitConfigRoute(PrivateGroup)   // 注册功能api路由

		router.InitWalletRoute(PrivateGroup) // 注册功能api路由+

		router.InitBankRechargeRoute(PrivateGroup)    // 注册功能api路由+
		router.InitUserWithdrawRoute(PrivateGroup)    // 注册功能api路由+
		router.InitAccountFundsRoute(PrivateGroup)    // 注册功能api路由+
		router.InitAccountFundsLogRoute(PrivateGroup) // 注册功能api路由+
		router.InitFundsRechargeRoute(PrivateGroup)   // 注册功能api路由+

		router.InitUserRoute(PrivateGroup) // 注册功能api路由+

		router.InitNewsRoute(PrivateGroup)      // 注册新闻模块
		router.InitAgreementRoute(PrivateGroup) // 注册协议模块
		router.InitVersionRoute(PrivateGroup)   // 注册版本管理模块

		router.InitTplRoute(PrivateGroup)             // 短信通知模板
		router.InitAccountMessageRoute(PrivateGroup)  // 用户消息
		router.InitAccountFeedbackRoute(PrivateGroup) // 用户反馈模块

		router.InitWebsiteBannerRoute(PrivateGroup) // 官网banner
		router.InitSettingRoute(PrivateGroup)       // 设置模块

		router.InitAccesslogRoute(PrivateGroup)         // 访问日志模块
		router.InitVercodeRoute(PrivateGroup)           // 验证码查看
		router.InitUserLevelRoute(PrivateGroup)         // 会员等级
		router.InitTradeAccountApplyRoute(PrivateGroup) // 注册功能api路由+

		router.InitWalletChainRoute(PrivateGroup)
		router.InitCategoryRoute(PrivateGroup)
		router.InitGoodsRoute(PrivateGroup)
		router.InitPlaysettingRoute(PrivateGroup)
		router.InitUserStoreRoute(PrivateGroup)
		router.InitGameLottyRecordRoute(PrivateGroup)
		router.InitLevelUpdateLogRoute(PrivateGroup)
		router.InitExpRecordRoute(PrivateGroup)
		router.InitOrdersRoute(PrivateGroup)
		router.InitOrderItemsRoute(PrivateGroup)
		router.InitPointRedeemRecordRoute(PrivateGroup)
		router.InitTbCouponRoute(PrivateGroup)
		router.InitTbCouponListRoute(PrivateGroup)
...
func LoadTemplate(templatesDir string) multitemplate.Renderer {
	...
	funcMap := template.FuncMap{
		...
		"eqpoint": func(v interface{}, values int) bool {
			...
			//fmt.Println(value, "vvvvvvvvvvvvvvvvvvvvvvvvvvv")
